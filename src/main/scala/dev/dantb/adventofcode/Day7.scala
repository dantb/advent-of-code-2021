...
object Day7:
  ...
  def computeWithFunction(input: List[Int], totalFuelByPositionOneWay: List[CrabPosition] => Map[Int, Int]): Long =
    ...
    val finalFuelByPosition = totalFuelByPositionForward.zip(totalFuelByPositionBackward).map { case ((_, forward), (_, backward)) => forward + backward }
    finalFuelByPosition.min

  def part1computeTotalFuelByPositionOneWay(crabPositions: List[CrabPosition]): Map[Int, Int] =
    crabPositions.tail.foldLeft((Map(crabPositions(0).position -> 0), 0, crabPositions(0), crabPositions(0).numberOfCrabs)) { 
      case ((totalFuelByPosition, fuelToThisPoint, previousCrab, crabsSoFar), nextCrab) =>
        ...
        val newTotalFuelByPosition = totalFuelByPosition.updatedWith(nextCrab.position)(_.map(_ + totalFuel).orElse(Some(totalFuel)))
  ...
  def part2ComputeTotalFuelByPositionOneWay(crabPositions: List[CrabPosition]): Map[Int, Int] =
    crabPositions.tail.foldLeft((Map(crabPositions(0).position -> 0), Map(crabPositions(0) -> 1), 0, crabPositions(0))) { 
